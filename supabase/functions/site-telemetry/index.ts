...
const allowedEvents = new Set([
  ...
  "command_nexus_download", "trial_key_claimed",
  ...
  "public_feedback_submitted", "bug_report_submitted", "support_request_submitted",
  "signup_submitted", "login_success", "login_failed", "password_reset_requested", "service_checkout_opened",
]);
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (ipHash && securityEvents.has(eventType)) {
      const since = new Date(Date.now() - 60_000).toISOString();
      const { count } = await admin.from("security_events").select("id", { count: "exact", head: true }).eq("ip_hash", ipHash).gte("created_at", since);
    ...
    if (securityEvents.has(eventType)) {
      const { error: securityError } = await admin.from("security_events").insert({
        event_type: eventType, severity, page_path: pagePath, visitor_token: visitorToken,
        user_id: user?.id || null, user_email: user?.email || null, attempted_email: attemptedEmail,
        ip_address: ip, ip_hash: ipHash, user_agent: userAgent, referrer, metadata,
      });
      if (securityError) throw securityError;
    }

    if (user?.id) {
      const existing = await admin.from("user_operator_profiles").select("first_seen_ip,first_seen_at,event_count,login_count").eq("user_id", user.id).maybeSingle();
      if (existing.data) {
        await admin.from("user_operator_profiles").update({
          first_seen_ip: existing.data.first_seen_ip || ip, last_seen_ip: ip, last_user_agent: userAgent,
          first_seen_at: existing.data.first_seen_at || new Date().toISOString(), last_seen_at: new Date().toISOString(),
